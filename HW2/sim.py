...
class SimGame:
  ...
  def draw_edge(self, a, b, color,graph):
    graph[a][color].append(b)
    graph[b][color].append(a)

    #print("Adding %s edge %s <--> %s" % ("Blue" if color else "Red", a, b))
  ...
  def get_valid_moves(self, color,graph):
    assert color == 0 or color == 1, "invalid color: %d" % color
    ...
    for node1 in self.nodes:
      for node2 in self.nodes:
        if node1 != node2:
          if self.validate_move(node1, node2, color,graph):
  ...
  def alpha_beta_search(self,depth,alpha,beta,color,graph):
    if depth == 0:
      return len(self.get_valid_moves(color,graph))
    if color == self.computer_color:
      v = -float('inf')
      children = self.get_valid_moves(color,graph)
      for child in children:
        new_graph = copy.deepcopy(graph)
        self.draw_edge(child[0],child[1],color,new_graph)
        v = max(v,self.alpha_beta_search(depth-1,alpha,beta,not color,new_graph))
        alpha = max(alpha,v)
        if beta <= alpha:
          break
      return v
      ...
      for child in children:
        new_graph = copy.deepcopy(graph)
        self.draw_edge(child[0],child[1],color,new_graph)
        v = min(v,self.alpha_beta_search(depth-1,alpha,beta,not color,new_graph))
        beta = min(beta,v)
        if beta <= alpha:
          break
      return v
  ...
  def get_move_ab(self,depth):
    print("depth",depth)
    children = self.get_valid_moves(self.computer_color,self.graph)
    random.shuffle(children)
    ab_values = []
    for child in children:
      ab_values.append(self.alpha_beta_search(depth,-float('inf'),float('inf'),self.player_color,self.graph))
  ...
  def make_move(self, color):
    ...
    possible_moves = self.get_valid_moves(color,self.graph)
    ...
    self.draw_edge(move[0], move[1], color, self.graph)


  # Query the user to get the color they want to play as
  ...
  def get_player_move(self):
    print("Valid Moves for %s: %s", self.player_color, self.get_valid_moves(self.player_color,self.graph))
  ...
  def play(self):
    ...
    while not self.game_over:
      print("Red move")
      if len(self.get_valid_moves(0,self.graph)) == 0:
        print("No valid moves for Red")
        self.winner = 1
        break
      if not self.player_color:
        a, b = self.get_player_move()
        self.draw_edge(a, b, self.player_color, self.graph)
      else:
        self.make_move(self.computer_color)

      print("Blue move")
      if len(self.get_valid_moves(1,self.graph)) == 0:
